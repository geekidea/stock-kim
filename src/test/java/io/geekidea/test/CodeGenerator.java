...
import io.geekidea.framework.generator.config.SpringBootPlusMySqlQuery;
import lombok.Data;
...
public class CodeGenerator {
     ...
     * 是否生成Add方法
     */
    private boolean generatorAddMethod = true;
    /**
     * 是否生成Add方法
     */
    private boolean generatorUpdateMethod = true;
    /**
     * 是否生成Update方法
     */
    private boolean generatorDeleteMethod = true;
    /**
     * 是否生成Info方法
     */
    private boolean generatorInfoMethod = true;
    /**
     * 是否生成Page方法
     */
    private boolean generatorPageMethod = true;
     ...
     */
    public void init() {
        this.commonParentPackage = this.parentPackage + ".framework.common";
        // 父类包路径
        ...
        this.superQueryParam = this.commonParentPackage + ".query.BaseQuery";
        this.superEntityCommonColumns = new String[]{};

        // 公共类包路径
        this.commonIdParam = this.commonParentPackage + ".query.IdQuery";
        this.commonApiResult = this.commonParentPackage + ".api.ApiResult";
        this.commonOrderEnum = this.commonParentPackage + ".enums.OrderEnum";
        this.commonOrderQueryParam = this.commonParentPackage + ".query.BaseOrderPageQuery";
    ...
    public void generator() {
        ...
        gc.setBaseResultMap(baseResultMap); // 生成resultMap
        ...
        InjectionConfig cfg = new InjectionConfig() {
            ...
            public void initMap() {
                ...
                // 查询参数包路径
                String queryParamPackage = parentPackage + StringPool.DOT + pc.getModuleName() + ".dto.query";
                map.put("queryParamPackage", queryParamPackage);
                // 查询参数类路径
                map.put("queryParamPath", queryParamPackage + StringPool.DOT + pascalTableName + "Query");
                // 查询参数共公包路径
                map.put("queryParamCommonPath", superQueryParam);
                // 查询参数共公包路径
                map.put("idParamPath", commonIdParam);
                // 响应结果包路径
                String queryVoPackage = parentPackage + StringPool.DOT + pc.getModuleName() + ".dto.vo";
                ...
                // 是否生成对应的方法
                map.put("generatorAddMethod", generatorAddMethod);
                map.put("generatorUpdateMethod", generatorUpdateMethod);
                map.put("generatorDeleteMethod", generatorDeleteMethod);
                map.put("generatorInfoMethod", generatorInfoMethod);
                map.put("generatorPageMethod", generatorPageMethod);
        ...
        if (generatorQueryParam) {
            focList.add(new FileOutConfig("/templates/queryParam.java.vm") {
                @Override
                public String outputFile(TableInfo tableInfo) {
                    return projectPath + "/src/main/java/" + projectPackagePath + "/" + pc.getModuleName() + "/dto/query/" + tableInfo.getEntityName() + "Query" + StringPool.DOT_JAVA;
                }
            });
        }
        ...
        if (generatorQueryVo) {
            focList.add(new FileOutConfig("/templates/queryVo.java.vm") {
                @Override
                public String outputFile(TableInfo tableInfo) {
                    return projectPath + "/src/main/java/" + projectPackagePath + "/" + pc.getModuleName() + "/dto/vo/" + tableInfo.getEntityName() + "QueryVo" + StringPool.DOT_JAVA;
                }
            });
        }
    ...
    public void onlyUpdateEntityAndVo(boolean flag) {
        if (flag) {
            ...
            this.generatorQueryParam = false;
            this.generatorEntity = true;
            this.generatorQueryVo = true;
        }
    }
}
